      ...
      >
        ...
        <div class="p-6">
          ...
          <form @submit.prevent="handleSubmit">
            ...
            <!-- Form Actions -->
            <div class="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 mt-6 pt-4 border-t border-gray-200">
              <button
                type="button"
                @click="closeModal"
                :disabled="loading"
                class="mt-3 sm:mt-0 w-full sm:w-auto inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Cancel
              </button>
              <button
...
const validateForm = (): Record<string, string[]> => {
  ...
  if (!form.prefix || !form.prefix.trim()) {
    ...
    if (form.prefix.length > 20) {
      validationErrors.prefix = ['The prefix must not exceed 20 characters.']
    } else if (!/^[a-zA-Z.\s]+$/.test(form.prefix)) {
      validationErrors.prefix = ['The prefix may only contain letters, dots, and spaces.']
    }
  }
  ...
  if (!form.first_name || !form.first_name.trim()) {
    ...
    if (form.first_name.length > 100) {
      ...
      validationErrors.first_name = ['The first name may only contain letters, spaces, hyphens, and apostrophes.']
    }
  }
  ...
  if (!form.last_name || !form.last_name.trim()) {
    ...
    if (form.last_name.length > 100) {
      ...
      validationErrors.last_name = ['The last name may only contain letters, spaces, hyphens, and apostrophes.']
    }
  }
  ...
  if (!form.mobile_number || !form.mobile_number.trim()) {
    ...
    if (form.mobile_number.length > 20) {
      validationErrors.mobile_number = ['The mobile number must not exceed 20 characters.']
    } else if (!/^[\+]?[0-9\s\-\(\)]+$/.test(form.mobile_number)) {
      validationErrors.mobile_number = ['The mobile number format is invalid. Only numbers, spaces, hyphens, parentheses, and plus sign are allowed.']
    }
  }
